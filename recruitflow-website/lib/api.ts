 ...
 * readable by proxy.ts. On the server (should this module ever run there) we use
 * the absolute backend URL, since a relative path has no host server-side.
 */
const API_URL =
...
export async function submitApplication(form: FormData): Promise<ApplicationSummary> {
  const res = await wakeFetch(`${API_URL}/applications`, {
    method: "POST",
    credentials: "include",
    headers: portalHeaders(),
    body: form,
...
export function openPipelineSocket(): WebSocket {
  ...
  // to the backend origin (no cookie needed for the read-only ATS stream).
...
export function getMyInterviews() {
  return apiFetch<InterviewRecord[]>("/interviews/my");
}
...
/** Upload a profile picture. Returns the updated user with new picture_url. */
export async function uploadProfilePicture(file: File): Promise<CurrentUser> {
  const form = new FormData();
  form.append("file", file);
  const res = await wakeFetch(`${API_URL}/auth/profile/picture`, {
    method: "POST",
    credentials: "include",
    headers: portalHeaders(),
    body: form,
...
/** Update the current user's gender (drives the default avatar). "" clears it. */
export function updateProfileGender(gender: "male" | "female" | "") {
